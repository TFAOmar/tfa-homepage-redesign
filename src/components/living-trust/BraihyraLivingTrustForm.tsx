...
const useShakeOnError = (form: ReturnType<typeof useForm<BraihyraLivingTrustFormData>>) => {
  ...
  useEffect(() => {
    const subscription = form.watch((_, { name, type }) => {
      if (type === "change" && name) {
        const fieldError = form.formState.errors[name as keyof BraihyraLivingTrustFormData];
        if (fieldError && form.formState.touchedFields[name as keyof BraihyraLivingTrustFormData]) {
          triggerShake(name);
        }
      }
    });
...
type BraihyraLivingTrustFormData = z.infer<typeof livingTrustFormSchema>;
...
export default function BraihyraLivingTrustForm() {
  ...
  const form = useForm<BraihyraLivingTrustFormData>({
  ...
  const onSubmit = async (data: BraihyraLivingTrustFormData) => {
    if (isBot()) {
      setIsSubmitted(true);
      return;
    }
    ...
    try {
      ...
      const response = await submitForm({
        form_name: "Living Trust Inquiry - Braihyra",
        first_name: data.firstName,
        last_name: data.lastName,
        email: data.email,
        phone: data.phone,
        notes,
        tags: ["Living Trust", "Braihyra Medellin", "Big Moves Financial"],
        advisor_slug: "braihyra-medellin",
        advisor_email: "bri@tfainsuranceadvisors.com",
      ...
      toast({
        ...
        description: "Braihyra will contact you within 24-48 hours.",
      });
    } catch (error) {
    ...
    return (
      <div className="bg-white/10 backdrop-blur-xl rounded-xl md:rounded-2xl border border-white/15 p-6 md:p-8 lg:p-12 text-center">
        ...
        <p className="text-white/70 max-w-md mx-auto text-sm md:text-base">
          Braihyra will review your information and contact you within 24-48 hours 
    ...
    <div className="bg-white/10 backdrop-blur-xl rounded-xl md:rounded-2xl border border-white/15 p-4 sm:p-6 md:p-8">
      <div className="text-center mb-6 md:mb-8">
        ...
        <p className="text-white/70 text-sm md:text-base">
          Fill out this quick questionnaire and Braihyra will be in touch.
        </p>
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 md:space-y-6">
          <div className={honeypotClassName}>
            <label htmlFor="braihyra_website">Website</label>
            <input
              type="text"
              id="braihyra_website"
              name="braihyra_website"
              {...honeypotProps}
            />
          </div>
                  ...
                  <FormControl>
                    <div className="relative">
                      ...
                      {fieldState.isDirty && !fieldState.error && (
                        <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-emerald-400 animate-pop-in motion-reduce:animate-none" />
                      )}
                    </div>
                  </FormControl>
          ...
          <FormField
            ...
            render={({ field, fieldState }) => (
              <FormItem>
                <FormLabel className="text-white/90 text-sm md:text-base">Additional Notes (Optional)</FormLabel>
                <FormControl>
                  <Textarea
                    placeholder="Any specific questions or concerns you'd like to discuss..."
                    className={cn(
                      getInputClasses(!!fieldState.error, false, shakingFields.has("notes")),
            ...
            render={({ field, fieldState }) => (
              <FormItem className={cn(
                ...
                <div className="space-y-1 leading-none">
                  <FormLabel className="text-white/80 text-sm md:text-base font-normal cursor-pointer">
                    I agree to be contacted by Braihyra Medellin regarding my Living Trust inquiry.
                  </FormLabel>
                  <FormMessage className="text-sm text-red-300 mt-1 animate-slide-down-fade motion-reduce:animate-none" />
                </div>
              </FormItem>
          ...
          <Button
            ...
            className="w-full bg-accent hover:bg-accent/90 text-primary font-semibold py-4 md:py-6 text-base md:text-lg rounded-xl shadow-lg hover:shadow-accent/25 transition-all duration-300 touch-manipulation min-h-[52px]"
          >
            {isSubmitting ? (
              <>
