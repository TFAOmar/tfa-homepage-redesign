...
const useShakeOnError = (form: ReturnType<typeof useForm<EscobarFormData>>) => {
  ...
  useEffect(() => {
    const subscription = form.watch((_, { name, type }) => {
      if (type === "change" && name) {
        const fieldError = form.formState.errors[name as keyof EscobarFormData];
        if (fieldError && form.formState.touchedFields[name as keyof EscobarFormData]) {
          triggerShake(name);
        }
      }
    });
...
const escobarFormSchema = z.object({
...
type EscobarFormData = z.infer<typeof escobarFormSchema>;

const getInputClasses = (hasError: boolean, hasSuccess: boolean, isShaking: boolean = false) => cn(
  "bg-white/5 border-[#C9A84C]/30 text-white placeholder:text-white/40 min-h-[44px] text-base transition-all duration-200 ease-out focus-visible:ring-[#C9A84C]/50 focus-visible:border-[#C9A84C]",
...
const getSelectClasses = (hasError: boolean, hasSuccess: boolean, isShaking: boolean = false) => cn(
  "bg-white/5 border-[#C9A84C]/30 text-white min-h-[44px] text-base transition-all duration-200 ease-out",
  hasError && "border-red-400",
  hasSuccess && "border-emerald-400",
  isShaking && "animate-shake motion-reduce:animate-none"
);

export default function EscobarLivingTrustForm() {
  ...
  const form = useForm<EscobarFormData>({
    resolver: zodResolver(escobarFormSchema),
    mode: "onTouched",
    defaultValues: {
      firstName: "",
      lastName: "",
  ...
  const onSubmit = async (data: EscobarFormData) => {
    if (isBot()) {
      setIsSubmitted(true);
      return;
    }
    ...
    try {
      ...
      const response = await submitForm({
        form_name: "Living Trust Inquiry - Escobar Realty Group",
        ...
        advisor_email: "info@tfainsuranceadvisors.com",
  ...
  if (isSubmitted) {
    return (
      <div className="bg-white/10 backdrop-blur-xl rounded-xl md:rounded-2xl border border-[#C9A84C]/30 p-6 md:p-8 lg:p-12 text-center">
        <div className="relative w-14 h-14 md:w-16 md:h-16 mx-auto mb-4 md:mb-6">
          <div className="absolute inset-0 bg-emerald-500/20 rounded-full animate-success-pulse motion-reduce:animate-none" />
          <div className="absolute inset-0 bg-emerald-500/20 rounded-full" />
          <div className="absolute inset-0 flex items-center justify-center">
            <CheckCircle className="w-7 h-7 md:w-8 md:h-8 text-emerald-400 animate-checkmark-appear motion-reduce:animate-none" />
          </div>
        </div>
        ...
        <p className="text-white/70 max-w-md mx-auto text-sm md:text-base">
          Our estate planning team will review your information and contact you within 24-48 hours 
  ...
  return (
    <div className="bg-white/10 backdrop-blur-xl rounded-xl md:rounded-2xl border border-[#C9A84C]/30 p-4 sm:p-6 md:p-8">
      <div className="text-center mb-6 md:mb-8">
        <h3 className="text-xl md:text-2xl font-semibold text-white mb-2">
          Request Your Free Consultation
        </h3>
        <p className="text-white/70 text-sm md:text-base">
          Fill out this quick questionnaire and our team will be in touch.
        </p>
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 md:space-y-6">
          <div className={honeypotClassName}>
            <label htmlFor="escobar_website">Website</label>
            <input type="text" id="escobar_website" name="escobar_website" {...honeypotProps} />
          </div>
              ...
              <FormControl>
                <RadioGroup onValueChange={field.onChange} defaultValue={field.value} className={cn(
                  ...
                  <div className="flex items-center space-x-2 min-h-[44px]">
                    <RadioGroupItem value="yes" id="esc-property-yes" className="border-white/40 text-[#C9A84C] w-5 h-5" />
                    <label htmlFor="esc-property-yes" className="text-white/90 cursor-pointer text-base">Yes</label>
                  </div>
                  <div className="flex items-center space-x-2 min-h-[44px]">
                    <RadioGroupItem value="no" id="esc-property-no" className="border-white/40 text-[#C9A84C] w-5 h-5" />
                    <label htmlFor="esc-property-no" className="text-white/90 cursor-pointer text-base">No</label>
                  </div>
                </RadioGroup>
              </FormControl>
              <FormMessage className="text-sm text-red-300 mt-1 animate-slide-down-fade motion-reduce:animate-none" />
            ...
            <FormItem>
              ...
              <FormControl>
                <Textarea
                  ...
                  className={cn(
                    "bg-white/5 border-[#C9A84C]/30 text-white placeholder:text-white/40 min-h-[100px] text-base transition-all duration-200 ease-out focus-visible:ring-[#C9A84C]/50",
            ...
            <FormItem className={cn(
              ...
              <FormControl>
                <Checkbox
                  ...
                  className={cn(
                    "border-white/40 data-[state=checked]:bg-[#C9A84C] data-[state=checked]:border-[#C9A84C] mt-0.5 w-5 h-5",
              ...
              <div className="space-y-1 leading-tight">
                <FormLabel className="text-white/70 text-xs md:text-sm font-normal cursor-pointer leading-relaxed">
                  I agree to be contacted by The Financial Architects regarding Living Trust services. 
          ...
          <Button
            ...
            className="relative w-full bg-[#C9A84C] hover:bg-[#B8973D] text-black font-semibold py-4 md:py-6 text-base md:text-lg rounded-full shadow-lg hover:shadow-[#C9A84C]/25 transition-all duration-300 touch-manipulation min-h-[52px] md:min-h-[60px] overflow-hidden"
          >
            {isSubmitting && (
