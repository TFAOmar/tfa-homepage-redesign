...
const useShakeOnError = (form: ReturnType<typeof useForm<ThinkTaxFormData>>) => {
  ...
  useEffect(() => {
    const subscription = form.watch((_, { name, type }) => {
      if (type === "change" && name) {
        const fieldError = form.formState.errors[name as keyof ThinkTaxFormData];
        if (fieldError && form.formState.touchedFields[name as keyof ThinkTaxFormData]) {
          triggerShake(name);
        }
      }
    });
...
const thinkTaxFormSchema = z.object({
...
type ThinkTaxFormData = z.infer<typeof thinkTaxFormSchema>;
...
export default function ThinkTaxLivingTrustForm() {
  ...
  const form = useForm<ThinkTaxFormData>({
    resolver: zodResolver(thinkTaxFormSchema),
    mode: "onTouched",
    defaultValues: {
      firstName: "",
      lastName: "",
  ...
  const onSubmit = async (data: ThinkTaxFormData) => {
    if (isBot()) {
      setIsSubmitted(true);
      return;
    }
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 md:space-y-6">
          <div className={honeypotClassName}>
            <label htmlFor="think_tax_website">Website</label>
            <input type="text" id="think_tax_website" name="think_tax_website" {...honeypotProps} />
          </div>
              ...
              <FormControl>
                <RadioGroup onValueChange={field.onChange} defaultValue={field.value} className={cn(
                  ...
                  <div className="flex items-center space-x-2 min-h-[44px]">
                    <RadioGroupItem value="yes" id="tts-property-yes" className="border-white/40 text-accent w-5 h-5" />
                    <label htmlFor="tts-property-yes" className="text-white/90 cursor-pointer text-base">Yes</label>
                  </div>
                  <div className="flex items-center space-x-2 min-h-[44px]">
                    <RadioGroupItem value="no" id="tts-property-no" className="border-white/40 text-accent w-5 h-5" />
                    <label htmlFor="tts-property-no" className="text-white/90 cursor-pointer text-base">No</label>
                  </div>
                </RadioGroup>
              </FormControl>
              <FormMessage className="text-sm text-red-300 mt-1 animate-slide-down-fade motion-reduce:animate-none" />
          ...
          <FormField control={form.control} name="notes" render={({ field, fieldState }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea
                  placeholder="Any specific questions about Living Trusts or Mortgage Protection?"
                  className={cn(
            ...
            <FormItem className={cn(
              ...
              <div className="space-y-1 leading-tight">
                <FormLabel className="text-white/70 text-xs md:text-sm font-normal cursor-pointer leading-relaxed">
                  I agree to be contacted by Vanessa Sanchez regarding Living Trust and Mortgage Protection services. 
