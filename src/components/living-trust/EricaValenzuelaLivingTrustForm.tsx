...
const useShakeOnError = (form: ReturnType<typeof useForm<EricaLivingTrustFormData>>) => {
  ...
  useEffect(() => {
    const subscription = form.watch((_, { name, type }) => {
      if (type === "change" && name) {
        const fieldError = form.formState.errors[name as keyof EricaLivingTrustFormData];
        if (fieldError && form.formState.touchedFields[name as keyof EricaLivingTrustFormData]) {
          triggerShake(name);
        }
      }
    });
...
type EricaLivingTrustFormData = z.infer<typeof livingTrustFormSchema>;
...
export default function EricaValenzuelaLivingTrustForm() {
  ...
  const form = useForm<EricaLivingTrustFormData>({
  ...
  const onSubmit = async (data: EricaLivingTrustFormData) => {
    if (isBot()) {
      setIsSubmitted(true);
      return;
    }
    ...
    try {
      ...
      const response = await submitForm({
        ...
        advisor_slug: "erica-valenzuela",
        advisor_email: "evalenzuela@tfainsuranceadvisors.com",
      ...
      toast({
        ...
        description: "Erica will contact you within 24-48 hours.",
      });
    } catch (error) {
    ...
    return (
      <div className="bg-white/10 backdrop-blur-xl rounded-xl md:rounded-2xl border border-white/15 p-6 md:p-8 lg:p-12 text-center">
        ...
        <p className="text-white/70 max-w-md mx-auto text-sm md:text-base">
          Erica will review your information and contact you within 24-48 hours 
    ...
    <div className="bg-white/10 backdrop-blur-xl rounded-xl md:rounded-2xl border border-white/15 p-4 sm:p-6 md:p-8">
      <div className="text-center mb-6 md:mb-8">
        ...
        <p className="text-white/70 text-sm md:text-base">
          Fill out this quick questionnaire and Erica will be in touch.
        </p>
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 md:space-y-6">
          <div className={honeypotClassName}>
            <label htmlFor="erica_website">Website</label>
            <input
              type="text"
              id="erica_website"
              name="erica_website"
            ...
            render={({ field, fieldState }) => (
              <FormItem>
                <FormLabel className="text-white/90 text-sm md:text-base">
                  Anything else you'd like to share? <span className="text-white/50">(Optional)</span>
                </FormLabel>
                <FormControl>
                  <Textarea
                    placeholder="Tell us about your situation or any questions you have..."
                    className={cn(
                      "bg-white/5 border-white/20 text-white placeholder:text-white/40 min-h-[100px] text-base resize-none",
          ...
          <FormField
            ...
            render={({ field, fieldState }) => (
              <FormItem className={cn(
                "flex flex-row items-start space-x-3 space-y-0 p-3 rounded-lg transition-all duration-200",
                fieldState.error && "bg-red-500/10 ring-1 ring-red-400",
                ...
                <FormControl>
                  <Checkbox
                    ...
                    className="border-white/40 data-[state=checked]:bg-accent data-[state=checked]:border-accent w-5 h-5 mt-0.5"
                  />
                </FormControl>
                <div className="space-y-1 leading-none">
                  <FormLabel className="text-white/80 text-sm md:text-base font-normal cursor-pointer">
                    I agree to be contacted by Erica Valenzuela regarding my Living Trust inquiry.
                  </FormLabel>
          ...
          <Button
            ...
            className="w-full bg-accent hover:bg-accent/90 text-primary font-semibold py-4 md:py-6 text-base md:text-lg rounded-full shadow-lg hover:shadow-accent/25 transition-all duration-300 touch-manipulation"
          >
            {isSubmitting ? (
              <>
