...
function buildPayload(lead: any, consent: any | null): Record<string, unknown> {
  ...
  return {
    ...
    solve_first: s(answers.solve_first ?? answers.anchor),
    ...
    trust_real_estate: s(answers.trust_real_estate ?? answers.real_estate),
    existing_estate_docs: s(answers.existing_estate_docs ?? answers.existing_docs),
    estate_composition: s(answers.estate_composition ?? answers.estate),
    prompting_event: s(answers.prompting_event ?? answers.why_now),
    nicotine: s(answers.nicotine),
    health_selfrating: s(answers.health_selfrating ?? answers.health),
    life_purpose: s(answers.life_purpose ?? answers.purpose),
    coverage_band: s(answers.coverage_band ?? answers.coverage),
    term_length: s(answers.term_length ?? answers.term),
    existing_life_coverage: s(answers.existing_life_coverage ?? answers.existing),
    retirement_window: s(answers.retirement_window ?? answers.window),
    work_situation_detail: s(answers.work_situation ?? answers.work),
    old_401ks: s(answers.old_401ks ?? answers.old_401k),
    savings_band: s(answers.savings_band ?? answers.savings),
    biggest_worry: s(answers.biggest_worry ?? answers.worry),
    monthly_saving: s(answers.monthly_saving ?? answers.save_monthly),
